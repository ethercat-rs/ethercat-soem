    ...
    fn coe_pdo_info(&mut self) -> Result<HashMap<ec::SlavePos, PdoInfo>> {
        ...
        for slave in 0..self.ctx.slave_count() as u16 {
            ...
            for sm in 2..=sm_cnt {
                ...
                if sm_type == 3 {
                    log::debug!("Slave {}: SM 3 (Outputs): read the assign RX PDO", slave);
                    pdo_info.extend_from_slice(&self.si_pdo_assign(slave_pos, sm)?);
                }
                if sm_type == 4 {
                    log::debug!("Slave {}: SM 3 (Inputs): read the assign TX PDO", slave);
                    pdo_info.extend_from_slice(&self.si_pdo_assign(slave_pos, sm)?);
                }
            }
        ...
        for pdo_sub in 1..=pdo_cnt {
            ...
            for entry_sub in 1..=pdo_entry_cnt {
                ...
                let pdo_entry_info = ec::PdoEntryInfo {
                    pos: ec::PdoEntryPos::new(
                        pdo_sub + entry_sub, //FIXME
                    ),
