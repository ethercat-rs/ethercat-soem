...
mod tests {
    ...
    #[test]
    fn ec_value_from_raw_byte_slice() {
        assert_eq!(
            value_from_slice(DataType::U16, &[123, 0]).unwrap(),
            Value::U16(123)
        );
        assert_eq!(
            value_from_slice(DataType::F32, &[0, 0, 128, 63]).unwrap(),
            Value::F32(1.0)
        );
    }

    #[test]
    fn ec_value_to_raw_byte_vec() {
        assert_eq!(value_to_bytes(Value::U16(123)).unwrap(), [123, 0]);
        assert_eq!(value_to_bytes(Value::F32(1.0)).unwrap(), [0, 0, 128, 63]);
    }
}
